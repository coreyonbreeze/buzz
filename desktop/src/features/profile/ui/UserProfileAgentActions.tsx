...
export function UserProfileAgentSettingsMenu({
  ...
  const hasPrimaryActions = Boolean(
    onDuplicatePersona || onExportPersona || onSaveAsTemplate,
  );
    ...
    <>
      <DropdownMenu modal={false}>
        ...
        >
          ...
          {onSaveAsTemplate ? (
            <DropdownMenuItem
              data-testid={`user-profile-save-as-template-${actionKey}`}
              disabled={isPending}
  ...
  if (viewerIsOwner && managedAgent) {
    return (
      <UserProfileAgentSettingsMenu
        ...
        // Only persona-less agents can be promoted to a template; a
        // persona-derived agent already has one behind it.
        onSaveAsTemplate={
          managedAgent.personaId === null ? onSaveAsTemplate : undefined
        }
